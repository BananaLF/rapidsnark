...
const std::string TEMP_FILE_DIR = "./build/temp_proof/";
...
void try_create_temp_dir() {
    try {
        ...
        if (fs::create_directory(TEMP_FILE_DIR)) {
            std::cout << "Directory ./build/temp_proof created successfully: " << TEMP_FILE_DIR << std::endl;
        } else {
            std::cout << "Directory ./build/temp_proof already exists " << TEMP_FILE_DIR << std::endl;
        }
    } catch (const fs::filesystem_error& e) {
